...
def test_memory_is_temporal():
    m = MemoryEntry(id="m.x", scope="char.x", type="fact", text="t", t_valid=5)
    assert isinstance(m, Temporal)


def test_visible_as_of():
    m = MemoryEntry(id="m.x", scope="char.x", type="fact", text="t", t_valid=5, t_invalid=10)
...
def test_no_invalid_means_always_valid_after_tvalid():
    m = MemoryEntry(id="m.y", scope="global", type="fact", text="t", t_valid=1)
