...
def test_append_get_persist(tmp_path):
    ...
    store: JsonStore[MemoryEntry] = JsonStore(MemoryEntry, path, key_field="id")
    ...
    # 重新加载后仍在（持久化）
    store2: JsonStore[MemoryEntry] = JsonStore(MemoryEntry, path, key_field="id")
    assert len(store2) == 2
